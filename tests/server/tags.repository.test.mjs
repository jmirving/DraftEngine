...
describe("tags repository bootstrap", () => {
  it("bootstraps default tag catalog before listTags", async () => {
    ...
    expect(queries[0].sql).toContain("INSERT INTO tags");
    expect(queries[0].params[0]).toHaveLength(BOOLEAN_TAGS.length);
    expect(queries[0].params[1]).toHaveLength(BOOLEAN_TAGS.length);
    expect(queries[1].sql).toContain("SELECT id, name, category");
  });
  ...
  it("bootstraps default tag catalog before allTagIdsExist checks", async () => {
    ...
    expect(query.mock.calls[0][0]).toContain("INSERT INTO tags");
    expect(query.mock.calls[1][0]).toContain("SELECT id");
  });
});
