...
- `NEXUS_EXCHANGE_URL` (Nexus grant exchange endpoint for hosted app launch; defaults to `<NEXUS_PORTAL_BASE_URL>/api/auth/exchange` when omitted)
...
- `NEXUS_EXCHANGE_SECRET` or `NEXUS_APP_EXCHANGE_SECRET` (generic fallback exchange secret names)
- `NEXUS_PORTAL_BASE_URL` (link target for hosted-auth error recovery; falls back to `NEXUS_PUBLIC_BASE_URL`)
